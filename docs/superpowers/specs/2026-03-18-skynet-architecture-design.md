...
基于 CFO Metrics 平台的界面分析，提取可复用的 UI/UX 设计模式。

### 布局结构

```
┌─────────────────────────────────────────────────────────────┐
│  顶部导航栏（面包屑 + 时间筛选器 + 帮助图标）                    │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌─────────────────┐ ┌─────────────────┐ ┌───────────────┐ │
│  │     模块 A      │ │     模块 B      │ │    模块 C     │ │
...
**布局特点**：
- **三列布局**：适合桌面端，可响应式调整为单列/双列
- **模块分组**：按业务域（风险质量、服务体验、财务等）组织指标
- **卡片式设计**：每个指标独立成卡，便于维护和扩展
...
| 用途 | 颜色 | 含义 |
|------|------|------|
| 品牌主色 | 蓝色 (#1890ff) | 导航、按钮、强调 |
| 正向指标 | 绿色 (#52c41a) | 超额完成、正向增长 |
| 负向指标 | 红色 (#ff4d4f) | 未达标、负向变化 |
| 警示/关注 | 黄色/橙色 (#faad14) | 接近阈值、需关注 |
| 中性/背景 | 灰色系 | 边框、分割线、背景 |
...
1. **仪表盘首页**：采用三列卡片布局，按业务域分组
2. **指标层级**：支持一级/二级/三级指标的树形导航
3. **目标管理**：每个指标配置目标值，自动计算完成率
4. **趋势可视化**：使用 Sparklines 库绘制迷你趋势图
5. **响应式设计**：移动端自动调整为单列布局
6. **主题切换**：支持亮色/暗色主题
